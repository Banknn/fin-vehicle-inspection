...
const DailyBillAccount = () => {
			...
			if (isValidResponse(res) || isValidResponse(res2)) {
				...
				const data = dataFilter.map((e, i) => {
					return {
						...
						sevice: +e.sevice,
						discount: +e.discount,
						discountPrb: +e.discount_prb,
						discountInsFee: +e.discount_inspection_fee,
						discountService: +e.discount_sevice,
	...
	const summaryRow = (pageData) => {
		...
		const discountPrb = pageData.reduce((acc, curr) => acc + curr.discountPrb, 0)
		const discountInsFee = pageData.reduce((acc, curr) => acc + curr.discountInsFee, 0)
		const discountService = pageData.reduce((acc, curr) => acc + curr.discountService, 0)
		...
		const discountPrbTotal = dataList.reduce((acc, curr) => acc + curr.discountPrb, 0)
		const discountInsFeeTotal = dataList.reduce((acc, curr) => acc + curr.discountInsFee, 0)
		const discountServiceTotal = dataList.reduce((acc, curr) => acc + curr.discountService, 0)
		...
		return (
			<TableComponent.Summary>
				<TableComponent.Summary.Row style={{ textAlign: 'center' }}>
					...
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountPrb || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountInsFee || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountService || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
				...
				<TableComponent.Summary.Row style={{ textAlign: 'center' }}>
					...
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountPrbTotal || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountInsFeeTotal || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(discountServiceTotal || '0', 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
...
export default DailyBillAccount
...
const ExportExcel = (columnsHead, dataList, summary, startDate, endDate) => {
	const handleClickExport = () => {
		...
		let newDataList1 = dataList.map((e) => [
			...
			convertStrToFormat(e.discountPrb || '0', 'money_digit'),
			convertStrToFormat(e.discountInsFee || '0', 'money_digit'),
			convertStrToFormat(e.discountService || '0', 'money_digit'),
		...
		const seviceTotal = dataList.reduce((acc, curr) => acc + curr.sevice, 0)
		const discountPrbTotal = dataList.reduce((acc, curr) => acc + curr.discountPrb, 0)
		const discountInsFee = dataList.reduce((acc, curr) => acc + curr.discountInsFee, 0)
		const discountService = dataList.reduce((acc, curr) => acc + curr.discountService, 0)
		const discountTotal = dataList.reduce((acc, curr) => acc + curr.discount, 0)
		const priceTotals = dataList.reduce((acc, curr) => acc + curr.priceTotal, 0)
