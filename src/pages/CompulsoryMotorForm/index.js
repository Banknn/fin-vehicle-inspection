...
const CompulsoryMotorForm = () => {
	...
	const validateFields = () => {
		...
		if (!select.companyCompulsory) {
			formIsValid = false
			errors['companyCompulsory'] = 'กรุณาเลือกบริษัทประกัน'
		}
		if (!select.compulsoryType) {
			formIsValid = false
			errors['compulsoryType'] = 'กรุณาเลือกประเภท พ.ร.บ.'
		}
		if (!workType) {
			formIsValid = false
			errors['workType'] = 'กรุณาเลือกประเภทงาน'
		}
		if (!brand) {
			formIsValid = false
		...
		if (
			!subSeries &&
			['ทิพยประกันภัย', 'แอกซ่าประกันภัย'].includes(select?.companyCompulsory)
		) {
			formIsValid = false
	...
	const handleClick = {
		...
		submit: async () => {
			...
			const brandTitle = getBrandTitle(companyCompulsory)
			const seriesTitle = seriesList.find((e) => e.value === series)
			const subSeriesTitle = subSeriesList.find((e) => e.value === subSeries)
			const countryTitle = getCountryList().find((e) => e.value === country)
			const nationalTitle = getNationalList().find((e) => e.value === national)

			const params = {
				quo_num: quoNum || null,
				company_prb: companyCompulsory,
				insuredType: legal === 'บุคคลทั่วไป' ? '1' : '2',
				prb_type: compulsoryType,
				policyStatus: workType,
				priceprb: prbPrice,
				brandplan: brand === 'none' ? brandOther : brandTitle?.text,
				yearplan: year === 'none' ? yearOther : year,
				registerYear: registerYear,
				seriesplan: series === 'none' ? seriesOther : seriesTitle?.text,
				sub_seriesplan: subSeries === 'none' ? subSeriesOther : subSeries,
			...
			let updatedCustomer = {
				...
				compulsoryType: compulsoryTypeTitle.text,
				isPassport,
				model: series,
				brandTitle,
				seriesTitle,
			...
			if (await handleClick.checkDuplicate()) {
				...
				if (isValidResponse(res)) {
					...
					if (isValidResponse(savePay)) {
						setFilesSuccess(companyCompulsory)
						dispatch(
							customerAction({
								...updatedCustomer,
								quoNum: newQuoNum,
	...
	const form = [
		{
			section: {
				...
				items: [
							...
							<Select
								...
								onChange={(value) => {
									...
									if (value === '1.10' || value === '1.10E') {
										...
										carTypes = 'รย.2'
										seats = '15'
									} else if (['1.40A', '1.40B', '1.40C'].includes(value)) {
										carTypes = 'รย.3'
										seats = '3'
										bodyTypes = 'รถบรรทุก'
									}
					...
					},
					{
						...
						item: (
							<Select
								...
								options={
									['ไทยศรีประกันภัย'].includes(select?.companyCompulsory)
										? [
												{
													value: 'N',
													text: 'งานใหม่',
					...
					{
						label: 'วันสิ้นสุดภาษี',
						required: true,
						row: true,
						hide:
							select?.companyCompulsory === 'แอกซ่าประกันภัย' && calDate() < 365
								? false
								: true,
						col: 6,
						item: (
							<DatePickerWithLabel
								name='วันสิ้นสุดภาษี'
								placeholder='วันสิ้นสุดภาษี'
								format='DD/MM/YYYY'
								value={taxExpireDate}
								onChange={(date) => {
									setTaxExpireDate(date)
								}}
							/>
						),
					},
			...
			section: {
				...
				items: [
					...
					},
					{
						label: 'รุ่นย่อยรถ',
						row: true,
						required: ['ทิพยประกันภัย', 'แอกซ่าประกันภัย'].includes(
							select?.companyCompulsory
						),
						col: 6,
						item: (
							<Select
								...
								disabled={
									!['ทิพยประกันภัย', 'แอกซ่าประกันภัย'].includes(
										select?.companyCompulsory
									)
								}
							/>
						...
						item: (
							<Input
								...
								disabled={
									...
									['หจก.', 'ห้างหุ้นส่วนจำกัด'].includes(select?.title)
								}
							/>
						),
					},
	...
	return (
		<DetailLayout
			...
			isStep={true}
		>
			<Label className='title-form'>แจ้งงาน พ.ร.บ.</Label>
			{form.map((e, i) => {
				const { section } = e
				return (
					<Box key={i}>
			...
			<Box className='accept-group-btn-wrapper'>
				...
				<Button
					className='accept-btn'
					width='140'
					onClick={handleClick.submitToInsurance}
				>
					ซื้อประกัน
				</Button>
				<Button
...
export default CompulsoryMotorForm
