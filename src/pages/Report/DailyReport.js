...
const DailyReport = () => {
			...
			if (isValidResponse(res) || isValidResponse(res2)) {
				...
				const data = dataFilter.map((e, i) => {
					...
					return {
						...
						com_prb: +e.show_com_prb,
						com_ins: +e.show_com_ins,
						other: otherPrices,
				...
				const comPrbPrice = data.reduce((acc, curr) => acc + curr.com_prb, 0)
				const comInsPrice = data.reduce((acc, curr) => acc + curr.com_ins, 0)
	...
	const handleClickExport = async () => {
		const startSelected = moment(startDate || new Date()).format('YYYY-MM-DD')
		const endSelected = moment(endDate || new Date()).format('YYYY-MM-DD')
		const start = moment(`${startSelected} 00:00:00`).format(
			'YYYY-MM-DD HH:mm:ss'
		)
		const end = moment(`${endSelected} 23:59:59`).format('YYYY-MM-DD HH:mm:ss')
		...
		const API = reportController()
		const res = await API.downloadDailyReport(params)
	...
	const summaryRow = (pageData) => {
		...
		const comPrbPrice = pageData.reduce((acc, curr) => acc + curr.com_prb, 0)
		const comInsPrice = pageData.reduce((acc, curr) => acc + curr.com_ins, 0)
		...
		return (
			<TableComponent.Summary>
				<TableComponent.Summary.Row style={{ textAlign: 'center' }}>
					...
					<TableComponent.Summary.Cell>
						{convertStrToFormat(comPrbPrice.toFixed(2), 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(comInsPrice.toFixed(2), 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
				...
				<TableComponent.Summary.Row style={{ textAlign: 'center' }}>
					...
					<TableComponent.Summary.Cell>
						{convertStrToFormat(summary.comPrbPrice.toFixed(2), 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
						{convertStrToFormat(summary.comInsPrice.toFixed(2), 'money_digit')}
					</TableComponent.Summary.Cell>
					<TableComponent.Summary.Cell>
...
const ExportExcel = (columnsHead, dataList, summary, startDate, endDate) => {
	const handleClickExport = () => {
		...
		let newDataList1 = dataList.map((e) => [
			...
			convertStrToFormat(e.com_prb || '0', 'money_digit'),
			convertStrToFormat(e.com_ins || '0', 'money_digit'),
		...
		dataCell.push(header1, ...newDataList1, [
			...
			convertStrToFormat(summary.comPrbPrice.toFixed(2) || '0', 'money_digit'),
			convertStrToFormat(summary.comInsPrice.toFixed(2) || '0', 'money_digit'),
